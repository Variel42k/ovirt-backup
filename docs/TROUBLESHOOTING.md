...
Откройте колонку «Худшая реплика». Каждое хранилище обязательно: сбой S3/SFTP
не закрывается успехом локальной копии. Исправьте именно указанное хранилище и
...
сервер мог не реализовать `statvfs`. Занятый объём и суточный рост при этом
