...
случайно обработать как oVirt. Proxmox проходит по отдельному клиенту, а его
не реализованные backup/restore возможности возвращают явную ошибку. Это граница для следующего драйвера:
сначала он объявляет собственное семейство и возможности, затем получает
явные ветки инвентаря, операций, backup и restore.
...
### Два backup-драйвера и отдельный Proxmox management-драйвер

oVirt и libvirt используют общий конвейер резервного копирования. Proxmox пока
участвует только в инвентаре, мониторинге и управлении, поэтому его возможности
backup/restore выключены в `/meta` и проверяются повторно на backend.
...
- **Межплатформенное восстановление.** Поддерживаются oVirt→oVirt и KVM→KVM;
  преобразование oVirt↔KVM не выполняется.
- **Backup/restore Proxmox.** Реализованы кластерный инвентарь, мониторинг,
  питание и миграция ВМ. Чтение и запись дисков Proxmox пока отсутствуют.
...
- **Host agent.** Наблюдение и управление идут через Engine или libvirt SSH;
  отдельного агента на гипервизорах нет.
