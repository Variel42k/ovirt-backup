...
закрывает на движке свои брошенные бэкапы и отменяет их передачи образов. После
каждого запуска, в том числе упавшего, уборка продолжается в фоне: каждые 10
минут, до 6 часов, служба закрывает свои брошенные бэкапы, отменяет зависшие
передачи и удаляет брошенные снапшоты — включая служебный снапшот, который
движок создал под бэкап. Так после аварии хоста диски разблокируются в течение
10 минут после его возвращения, а не при следующем запуске по расписанию. Если
за 6 часов разобрать не удалось, в карточке запуска появляются готовые команды.
