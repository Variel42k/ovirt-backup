...
func TestServerJSONOnlyReportsTrustMaterialPresence(t *testing.T) {
	...
	body := string(raw)
	if strings.Contains(body, "BEGIN CERTIFICATE") || strings.Contains(body, "ssh-ed25519") ||
		strings.Contains(body, `"ca_cert"`) || strings.Contains(body, `"ssh_host_key"`) {
		t.Fatalf("trust material leaked in JSON: %s", body)
	}
	if !strings.Contains(body, `"ca_cert_stored":true`) || !strings.Contains(body, `"ssh_host_key_stored":true`) {
