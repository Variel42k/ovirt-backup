...
Прямой HTTPS с автоматически выпущенным самоподписанным сертификатом:

```text
--url https://10.20.30.40:8080 --port 8080
```
...
Для `https://` установщик по умолчанию включает собственный TLS и выпускает
самоподписанный сертификат. Доверенную пару задают через `--tls files`, а при
завершении TLS на reverse proxy выбирают соответствующий пункт диалога:
процесс приложения тогда слушает только `127.0.0.1`. Сетевой `http://`
установщик отклоняет, если не переданы одновременно `--allow-http --tls none`;
это исключение предназначено для изолированного тестового стенда.

В режиме без диалога `--url` обязателен. В интерактивном режиме установщик
предложит `https://<первый-адрес-сервера>:<порт>`.
...
| `--allow-http` | разрешить сетевой HTTP только вместе с `--tls none`; исключение для лаборатории |
...
#### Запуск встроенного Keycloak из web

Docker-установка из `.run` ставит socket-activated службы
`jhvirt-keycloak-helper.socket` и `jhvirt-keycloak-helper.service`. Поэтому
Keycloak можно не выбирать при первоначальной установке, а включить позже в
**Настройки → Keycloak и домен** из сессии локального администратора:

1. В карточке «Встроенный Keycloak» укажите публичный HTTPS URL, порт, realm и
   OIDC client ID. Для штатного прямого TLS используется сертификат приложения.
2. Повторно введите пароль текущего локального администратора и нажмите
   «Запустить и подключить».
3. Helper создаст отдельную роль и БД PostgreSQL, включит Compose-профиль
   `keycloak`, дождётся `/realms/master`, создаст realm, группы и OIDC-клиента.
   Приложение проверит discovery, сохранит client secret зашифрованным и применит
   OIDC без перезапуска.
4. На втором шаге можно сразу подключить AD. Для LDAPS с корпоративным CA
   загрузите PEM bundle; интерфейс показывает только наличие выбранного файла.
   Bind-пароль попадёт в file vault, а в БД Keycloak останется `${vault.ad-bind}`.

Административный client ID и secret для встроенного Keycloak вводить не нужно:
их хранит root-only helper. Если Keycloak был создан предыдущей версией
установщика, helper один раз создаёт offline recovery-client, принимает
существующую БД и удаляет этот client после выпуска постоянной служебной записи.

Приложение не получает Docker socket. Оно видит только
`/run/jhvirt-host/keycloak-helper.sock`; helper принимает три фиксированные
операции: статус, запуск/настройка встроенного Keycloak и подключение домена.
Состояние и служебные секреты лежат в `/opt/jhvirt/keycloak-helper/keycloak.json`
с режимом `0600` и входят в migration archive. В systemd-варианте приложения
встроенный Keycloak не запускается: используйте внешний OIDC-провайдер.

Bind mounts truststore и file vault помечаются Compose как `ro,z`: `ro`
запрещает запись из контейнера, а общий SELinux label `z` разрешает Keycloak
читать их на enforcing-хостах Rocky/RHEL. Удаление `z` приводит к restart loop
Keycloak ещё до открытия публичного порта.

Helper передаёт `keycloak.conf` во вспомогательный контейнер через открытый
stdin Docker и сразу считывает файл обратно. Если Docker не принял содержимое,
bootstrap завершается до запуска Keycloak вместо старта с пустой конфигурацией.
После создания realm helper явно назначает постоянной служебной записи набор
административных ролей клиента `<realm>-realm`. Это назначение повторяется
идемпотентно. Глобальная роль `admin` в master у постоянной записи отсутствует;
при обновлении helper отзывает её у записей, созданных ранними версиями. После
успешной проверки также удаляются все одноразовые пользователи
`kc-web-bootstrap-*` и клиенты `kc-web-recovery-*`.

Проверка на хосте:

```bash
systemctl status jhvirt-keycloak-helper.socket
systemctl status jhvirt-keycloak-helper.service
docker compose -f /opt/jhvirt/compose/docker-compose.yml \
  --env-file /opt/jhvirt/compose/.env ps keycloak
```
...
приложения. Отдельно создаётся `backup-admin` в realm `jhvirt`, включается в
`virt-admins` и получает другой случайный пароль. Именно эта вторая запись
входит через кнопку Keycloak. Имя меняется параметром
`--keycloak-app-admin-user`; значение `none` отключает автоматическое создание.
...
Дальше в Keycloak можно завести остальных пользователей и включить их в
группы. Не попавший ни в одну не допускается в систему вовсе — это и есть
...
Команда останавливает Keycloak на время штатной offline recovery-операции,
задаёт новый случайный пароль, отзывает прежние сессии и печатает пароль один
раз. Realm `jhvirt`, пользователи, группы, federation, MFA и OIDC-клиент при
этом не пересоздаются. После входа откройте административную консоль по адресу,
который установщик выводит как `Keycloak`, и меняйте конфигурацию там.
...
env, база, пользователи и ключ шифрования бэкапов сохраняются, меняется только
...
`probe` отвечает версией протокола: `jhvirt-pve-data-plane/2`. На узлах
Proxmox VE 8.2 и новее следом идёт строка `fleecing`. Служба принимает и
помощник первой версии, но передать ему предел чтения и fleecing не может и
пишет об этом в хронологии запуска. После обновления службы переустановите
помощник на всех узлах той же командой `install`.
...
Необязательное поле **Хранилище для fleecing**, например `local-lvm`, включает
fleecing при бэкапе ВМ. Пока `vzdump` читает диск, старые блоки, которые гость
перезаписывает, ложатся в образ на этом хранилище узла, а не ждут, пока уйдут
по SSH к службе. Хранилище должно существовать на каждом узле; лучше быстрое
локальное: LVM-thin, ZFS, RBD. Подробнее — в
[HOT-BACKUP.md](HOT-BACKUP.md#6-fleecing-на-proxmox).
...
### Файловые бэкапы после установки

Функция доступна сразу и не требует включения `file_backup.enabled`.
Установщик создаёт именованный корень `default`:

| Режим | Источник | Каталог восстановления |
|---|---|---|
| Docker Compose из `.run` | `/opt/jhvirt/file-sources` | `/opt/jhvirt/file-restores` |
| Docker Compose из репозитория | `deploy/file-sources` | `deploy/file-restores` |
| systemd | `/opt/jhvirt/file-sources` | `/opt/jhvirt/file-restores` |

Скопируйте исходные данные в каталог источника, затем откройте **Файловые
бэкапы → Новое задание**. В Docker источник смонтирован в контейнер только для
чтения, каталог восстановления — для записи.

Для Docker внешние каталоги задаются в `.env` до запуска установщика:

```dotenv
JHV_FILE_BACKUP_DIR=/srv/file-sources
JHV_FILE_RESTORE_DIR=/srv/file-restores
```

Дополнительные корни задаются в `file_backup.roots`. Каждый такой путь должен
быть отдельно смонтирован в контейнер либо разрешён в `ReadOnlyPaths`/
`ReadWritePaths` unit-файла systemd. Флаг `file_backup.enabled` сохранён только
для чтения старых конфигураций и больше не отключает функцию.
...
Сами каталоги с бэкапами и восстановленными образами не помещаются в архив:
