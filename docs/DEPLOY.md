...
| `--uninstall` | снять приложение, сохранив конфигурацию, ключ, БД и данные |
...
1. создаёт `.env` с правами `0600`;
2. генерирует пароли PostgreSQL и первого администратора;
3. создаёт каталоги `backups` и `restores`;
4. собирает образ и запускает приложение с PostgreSQL;
5. ждёт `http://127.0.0.1:<порт>/readyz` до трёх минут;
6. удаляет bootstrap-пароль администратора из `.env`;
7. печатает адрес, логин `admin` и одноразово показанный пароль.
...
Интерактивно запустите установщик без аргументов и выберите **удалить**. Перед
действием будет отдельное подтверждение.

Для автоматизации:

```bash
sudo sh /tmp/jhvirt-*.run --uninstall
```
...
Удаляются код приложения и systemd unit, контейнеры останавливаются. Намеренно
сохраняются:
