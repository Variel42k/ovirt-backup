...
Достаточно любого одного канала; при `enabled: true` без единого настроенного
канала служба не запустится — молчание, выглядящее как работающая доставка,
хуже явного отказа.

**Сообщение уходит один раз** — когда оповещение загорелось либо загорелось
снова после того, как его погасили. Это не мелочь: монитор повторяет одну и ту
же беду каждые полминуты, и на стенде у одного оповещения накопилось тринадцать
тысяч повторов. Письмо на каждый повтор — надёжный способ добиться, чтобы канал
перестали читать целиком.

`min_severity: critical` по умолчанию по той же причине: предупреждений много,
и будить человека они не должны.

Доставка ничего не задерживает: сообщения уходят в очередь и отправляются
отдельным потоком. Если канал отвечает медленнее, чем приходят события, очередь
переполняется и сообщения теряются — с записью в журнал. Задерживать опрос
гипервизоров ради отправки писем нельзя.

Недоступность одного канала не мешает остальным: ошибка попадает в журнал,
повторов нет. Само оповещение никуда не делось и видно в интерфейсе — очередь
повторных отправок при недоступном SMTP превратилась бы в лавину.
