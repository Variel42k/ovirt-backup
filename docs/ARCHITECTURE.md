...
Подробные схемы — в [diagrams/jhvirt-architecture.drawio](diagrams/jhvirt-architecture.drawio):
14 вкладок от контекста системы до защиты СУБД. Слой «Защита» на
каждой вкладке показывает, что и чем защищено, а оранжевые метки — остаточные
риски; скрыть слой можно через «Вид → Слои» (Ctrl+Shift+L).
...
Сервис резервного копирования и присмотра за виртуальной инфраструктурой:

- **oVirt / РЕД Виртуализация** — через REST API движка и `ovirt-imageio`;
- **Proxmox VE** — кластерный инвентарь и управление QEMU/LXC через REST API,
  полный нативный backup/restore через ограниченный SSH data plane;
- **голый libvirt/KVM** — через libvirt поверх SSH и NBD.

Запись oVirt-семейства представляет полный Engine scope, поэтому один опрос
получает все кластеры и их узлы. Список коннекторов и их возможностей хранится
на backend и публикуется через `/meta`; неизвестный тип нельзя сохранить или
случайно обработать как oVirt. Proxmox проходит по отдельным REST-клиенту и
SSH data plane; нативный архив не попадает в oVirt-драйвер. Это граница для
следующего драйвера: сначала он объявляет собственное семейство и возможности,
затем получает явные ветки инвентаря, операций, backup и restore.
...
```
        ...
        ├──────────────────────────────────────────────────────────┤
        │  ovirt      proxmox      libvirtx     repo       store     │
        │  imageio    pkg/nbd      retention   logging    events    │
        └───┬────────────┬───────────┬────────────┬─────────────────┘
            │            │           │            │
            ▼            ▼           ▼            ▼
      движок oVirt  Proxmox API  libvirt+SSH  хранилище PostgreSQL
      443 / 54322   8006 + SSH   NBD          копий
                                  local/S3/SMB/
                                  WebDAV/SFTP
```
...
| `internal/proxmox` | API token, кластерный инвентарь Proxmox, питание, миграция и ограниченный SSH-поток `vzdump`/restore QEMU/LXC |
...
| `internal/dbdump` | логические дампы СУБД: SSH-канал к хелперу `jhvirt-db-dump`, поток в общий формат, копии, ретенция, восстановление в новую базу |
...
| `internal/monitor` | опрос состояния, I/O ВМ во время бэкапа, оповещения, восстановительные действия |
...
`backup`/`kvm` → `repo`/`ovirt`/`proxmox`/`libvirtx` → `store` → `model`.
...
| Задания ВМ, Engine, файлов и дампов СУБД | cron из базы в системной IANA-зоне | `scheduler` |
| Персистентные work units бэкапа | постоянно, по `priority` и времени | `scheduler` + PostgreSQL |
| Репликация обязательных копий | постоянно, с leases и heartbeat | `replication` |
| Внешние уведомления | постоянно, durable outbox по каналам | `notify` |
...
| Work units одного задания | `backup_jobs.concurrency` поверх глобального лимита | одно задание не должно занять все worker-ы |
| Повторных запусков задания | открытые `backup_tasks` и заявка `claimJob` | задание не должно запускаться поверх себя |
...
Запись о работе и её `backup_tasks` создаются **до** ожидания. Задачи сортируются
по убыванию `priority`, затем по времени создания, берутся через
`FOR UPDATE SKIP LOCKED` и получают lease с heartbeat. Поэтому очередь видна
оператору, переживает смену ведущего и не запускает одну ВМ двумя worker-ами.
Проверки и восстановления также регистрируют run до тяжёлой фоновой операции.
...
| Инвентарь | `vms`, `disks` |
| Копии ВМ | `storage_targets`, `backup_jobs`, `backup_job_runs`, `backup_tasks`, `backup_runs`, `backup_disks`, `backup_copies`, `repository_artifacts` |
| Engine и файлы | `engine_config_jobs`, `engine_config_runs`, `file_backup_jobs`, `file_backup_runs` |
| Проверка и возврат | `verify_runs`, `restore_runs` |
| Присмотр | `alerts`, `notification_policies`, `notification_deliveries`, `health_samples`, `remediation_records`, `remediation_periods` |
| Метрики | `disk_samples`, `mount_samples` |

**База — не источник истины о содержимом дисковых копий.** Она хранит задания,
очереди и оперативный каталог, но `run.json` и манифесты самодостаточны. После
потери базы данные читаются `jvbackup`; web-каталог умеет просканировать
репозиторий и явно импортировать непротиворечивые точки с родителями.
...
```
задание/кнопка → scheduler → backup_tasks (priority, lease) → dispatch
    ├─ oVirt:   Backup API → checkpoint → imageio (54322/54323) → чанки
    └─ libvirt: DomainBackupBegin → NBD → чанки
                                          │
                     io.Pipe → storage_mode → repo (local/S3/SMB/WebDAV/SFTP)
                                          │
                         data + manifest + последним run.json
```
...
подтверждения), новый диск в домене хранения или новая ВМ. Полное восстановление
поддерживает oVirt→oVirt и KVM→KVM. Preview сверяет ресурсы цели и сеть; каждый
NIC сопоставляется явно, по умолчанию отключён, а MAC генерируется заново.
Исполнитель записывает фазу и прогресс в `restore_runs`, а при ошибке удаляет
созданные диски/volumes и ВМ в обратном порядке. Межплатформенной конвертации нет.
...
### Производные и server-level артефакты

`export_qcow2` не меняет основной формат. После публикации дисковой точки
движок собирает каждый диск, вызывает `qemu-img`, снова делит результат на
чанки и регистрирует `repository_artifacts`. Такой QCOW2 имеет собственные
checksum, статус и ключи, проходит проверку, репликацию и ретенцию вместе с
точкой. Перед запуском наличие `qemu-img` проверяется явно.

Снимок oVirt Engine — отдельное server-level задание. Оно собирает доступные
секции конфигурации, сохраняет checksum и число отсутствующих секций, может
шифроваться и участвует в собственной ретенции. Web умеет просмотреть,
сравнить и скачать снимки; автоматического применения к Engine нет.

OVA стоит за другой границей: Engine записывает файл в указанный каталог
гипервизора и не предоставляет поток для помещения в репозиторий. Сервис хранит
состояние и путь внешнего артефакта, но не обещает ему обычную репликацию,
проверку или очистку.

### Нативный файловый бэкап

Файловый движок работает всегда и не смешивается с заданиями ВМ. Без
дополнительной настройки установщик предоставляет корень `file-sources` и
отдельную область `file-restores`. Дополнительные source/restore roots
администратор задаёт в YAML, а web/API оперируют только ID корня и
относительными include/exclude. Сканер не следует по symlink,
сохраняет symlink и hardlink как отдельные записи и проверяет canonical path.

```mermaid
flowchart LR
    A[allowlisted root] --> S[сканер include/exclude]
    S --> M[метаданные файлов, каталогов и ссылок]
    S --> C[чанки обычных файлов]
    C --> R[(общий repo backend)]
    M -->|commit marker последним| R
    R --> X[выборочное restore только в restore root]
```

Обычный файл использует тот же `DiskManifest`, сжатие, AES-GCM и backend, что и
диск ВМ. Инкремент повторно использует неизменившиеся данные. Изменившийся во
время чтения файл перечитывается один раз; повторно нестабильный путь оставляет
запуск `partial` с явным списком. Перезапись при восстановлении выключена по
умолчанию.
...
| `qemu` | `qemu-img check` по экспортированному qcow2 | нужен `qemu-img`; в установке из `.run` его нет |
...
Vue 3 + Quasar, собирается в статику и раздаётся тем же процессом. Интерфейс
покрывает инвентарь, задания ВМ/Engine/файлов, точки и физические копии,
восстановление, защиту, ретенцию, оповещения, хранилища, настройки, аудит и
встроенную техническую документацию.
...
Навигация сгруппирована по рабочим задачам. Состояние фильтров и открытых
разделов хранится в query-параметрах. `GlobalSearch` объединяет локальный
инвентарь с поиском по заданиям, копиям и оповещениям. `OperationCenter`
сводит серверные бэкапы, восстановления и репликации с действиями, начатыми
в текущем браузере. Длительная настройка Keycloak и домена использует отдельный
ограниченный контекст на сервере, поэтому закрытие вкладки не обрывает изменение
на середине; после возврата интерфейс сверяет сохранённое состояние.

На узких экранах основные таблицы становятся карточками с меню действий.
Создание задания и восстановление разбиты на шаги с проверкой каждого перехода.
Массовый выбор используется для запуска нескольких заданий, проверки нескольких
копий и создания одной политики для набора ВМ.

Живые обновления приходят по SSE из `internal/events`: прогресс бэкапов,
проверок, восстановлений и репликации, смена состояний, новые оповещения и
runtime-настройки. После разрыва интерфейс перечитывает состояние; отдельные
долго живущие экраны сохраняют fallback polling, потому что SSE намеренно не
является durable event log.
...
| Файловый бэкап | ID allowlisted root + относительные пути; symlink не обходится |
...
### Уведомления: политика отдельно от доставки

Монитор обновляет дедуплицированное `alert`, а планировщик уведомлений решает,
нужны ли `opened`, `reminder` или `resolved` по глобальной и per-kind политике.
Для каждого выбранного канала в той же базе создаётся отдельная outbox-строка.
SMTP, Telegram и webhook затем разбираются независимо: отказ почты не повторяет
уже принятый webhook.

Transport retry и напоминание — разные механизмы. Ошибка доставки повторяется
по короткому расписанию максимум пять раз; reminder создаёт новое событие с
новым `sequence` только через настроенный оператором интервал. Lease переживает
падение worker-а. Семантика at-least-once: редкий дубль после приёма внешней
системой и падения до фиксации возможен, поэтому webhook получает устойчивую
тройку `alert_id/event/sequence` для дедупликации.
...
Ручное применение использует токен предпросмотра. Токен считается по всему
набору сохраняемых и удаляемых точек; появление новой копии между просмотром и
подтверждением останавливает удаление и требует построить план заново. Тот же
токен входит в идентификатор заявки на согласование, поэтому подтверждение для
одной ВМ или старого состава копий нельзя перенести на другой план.
...
oVirt и libvirt сохраняют логические диски в общей сетке чанков. Proxmox
создаёт самодостаточный нативный `vzdump`-архив и передаёт его из owning node
одним потоком через SSH. Архив всё равно проходит через общий `DiskWriter`:
получает чанковые SHA-256, опциональное AES-256-GCM, реплики, retention и
`run.json`. Повторное сжатие выключено, потому что `vzdump` уже выдаёт zstd.

REST API и SSH разделены. API token читает весь кластер и выполняет разрешённые
операции управления. Приватный SSH-ключ хранится отдельно, а на каждом узле
его публичная часть привязана к `jhvirt-pve-data-plane` через forced command.
Helper принимает только `probe`, `backup` и `restore` с проверенными
аргументами; произвольную оболочку и отдельную команду удаления он не даёт.

Граница между backup-драйверами проходит по получению данных:

| Общее для всех | Своё у каждого |
|---|---|
...
- **Мгновенный запуск production-ВМ из репозитория.** Проверка `boot` и обычное
  восстановление сначала собирают образы; NBD-сервера с writable overlay и
  фоновым переносом пока нет.
- **Межплатформенное восстановление.** Поддерживаются oVirt→oVirt, KVM→KVM и
  Proxmox→Proxmox; преобразование между семействами не выполняется.
- **Инкрементальный Proxmox.** Доступен полный нативный `vzdump` в snapshot
  mode. Поток изменённых блоков и выборочное исключение дисков отсутствуют.
- **Проверка сценариев СУБД из службы.** Уровень `application` дают сценарии
  `fsfreeze-hook` внутри гостя; служба команд в госте не выполняет и видит
  только итог заморозки. Установлены ли сценарии, она проверить не может.
- **Инкрементальные и выборочные дампы СУБД.** Логический дамп всегда полный;
  PITR по WAL/binlog и восстановление отдельных таблиц через службу нет —
  `pg_restore` по файлу дампа выполняет администратор СУБД.
- **Host agent.** Полноценного агента нет. На Proxmox устанавливается небольшой
  root helper с фиксированным протоколом для потока `vzdump`; он не слушает порт
  и запускается только как forced command существующего `sshd`.
...
- **Нет прямого LDAP bind.** Внешний вход поддерживается через OIDC (например,
  Keycloak) с сопоставлением групп; LDAP подключается на стороне провайдера.
...
- **Live migration KVM-ВМ недоступна.** Одиночное KVM-подключение не описывает
  второй хост и общее хранилище, в отличие от кластера под Engine.
