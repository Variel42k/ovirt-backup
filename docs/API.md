...
| `retention_plan_changed` | 409 | состав копий изменился после предпросмотра ретенции |
...
Повторная проверка локального пароля ограничена тем же возрастающим cooldown,
что и обычный вход. Смена issuer, backchannel, client ID, client secret,
redirect URL, claim групп или ролевого mapping сбрасывает признак успешной
проверки домена. Локальный вход нельзя выключить, пока домен заново не проверен
для этой конфигурации; выключить его одновременно с самим OIDC также нельзя.
При таком изменении и после повторной настройки LDAP все существующие
...
сохранения больше его не получает; SHA-256 нужно сверить с доверенным
источником на стороне движка, поскольку само получение выполняется до
установления доверия.

Ручное подключение Proxmox использует `kind: "proxmox"`, `engine_url` с
...
паролем и запросы с bearer token не могут быть повторены на адрес из `Location`.
Если недоверенный движок отразит пароль или текущий token в тексте ошибки,
backend удалит их перед ответом, журналом и сохранением состояния подключения.
...
Подключение относится ко всему Engine, а не к одному гипервизору: после
сохранения служба сразу импортирует все кластеры, хосты, ВМ, диски и домены
хранения. Поле `kind` принимает `ovirt`, `redvirt`, `olvm` или `rhv` и сохраняет
фактический продукт при общей совместимой схеме API.
...
`GET /meta` публикует `virtualization_kinds`: список встроенных коннекторов,
семейство протокола, охватываемый уровень (`engine` или `host`) и отдельные
признаки поддержки backup, restore, снимка конфигурации, управления ВМ/узлами
и безопасного мастера.
Интерфейс строит выбор продукта по этому контракту. Неизвестный `kind`
отклоняется и никогда не передаётся клиенту другого семейства API.
...
`POST /retention/preview` возвращает `token` вместе со списками `keep` и
`delete`. Для применения тот же токен передаётся полем `plan_token`. Если между
просмотром и подтверждением появился новый бэкап или изменилась цепочка, сервер
отклоняет запрос до удаления данных: оператору нужно построить и проверить новый
план. Отрицательные значения всех полей политики отвергаются обоими маршрутами.
Причина передаётся query-параметром `reason`; при включённом согласовании она
обязательна и попадает в заявку. Ответ `202 approval_required` означает, что
данные ещё не удалены. Заявка привязана к подключению, ВМ, хранилищу и токену
плана; её нельзя использовать для другого набора копий.

При ошибке удаления отдельных копий операция возвращает ошибку с числом уже
обработанных точек и первым сбоем. Поэтому частичное применение не выглядит в
интерфейсе как полный успех; повторный предпросмотр показывает актуальный остаток.
...
`confirm: true`. Действия ВМ принимают `scope: "vm"`, действия хоста —
`scope: "host"`, переподключение движка — `scope: "server"`; смешивать тип
объекта и действие нельзя. `vm_reset` доступен только как явное ручное действие
и никогда не запускается автоматикой.
