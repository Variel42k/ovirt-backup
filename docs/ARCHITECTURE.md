...
SQLite открывается в режиме WAL и **с одним соединением** (`MaxOpenConns=1`).
Второе соединение не ускорило бы ничего: писатель у SQLite всё равно один, а
конкуренция за него превратилась бы в `SQLITE_BUSY` и повторы. С одним
соединением доступ сериализует пул Go — очередь вместо ошибок. Цена этого
решения — потолок пропускной способности, но упирается в него не бэкап:
копирование дисков базу не трогает, в неё идут короткие записи о состоянии.
