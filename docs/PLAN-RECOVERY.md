...
Первый этап реализован: задание заявляет уровень `crash`, `filesystem` или
`application`, запуск хранит достигнутый уровень и причину понижения, а
неудачная заморозка либо прерывает запуск до начала бэкапа, либо поднимает
оповещение `backup_consistency_degraded`. Уровень `application` обеспечивают
именованные сценарии внутри гостя (`deploy/guest-hooks`) через штатный
механизм `fsfreeze-hook` агента, а не произвольная команда из web. Подробности —
в [OPERATIONS.md](OPERATIONS.md#согласованность-копий-субд).

Второй этап — логические дампы СУБД — тоже реализован отдельным типом
заданий: forced command `jhvirt-db-dump` на хосте СУБД по образцу Proxmox,
поток `pg_dump`/`mysqldump` в общий формат хранения, восстановление только в
новую базу ([OPERATIONS.md](OPERATIONS.md#логические-дампы-субд)).
...
внутри той же платформы после стендовой приёмки. Мгновенный запуск, логические
дампы СУБД, host agent и межплатформенная конвертация остаются отдельными
функциями и не должны подразумеваться в интерфейсе или эксплуатационных
обещаниях.
